...
func (s *inMemoryOrdersStorage) GetAllOrdersForUser(ctx context.Context, userID int) mo.Result[[]dsmodels.Order] {
...
func (s *inMemoryOrdersStorage) UpdateOrder(ctx context.Context, order dsmodels.Order) mo.Result[dsmodels.Order] {
...
func (s *inMemoryOrdersStorage) InsertOrder(ctx context.Context, order dsmodels.Order) mo.Result[dsmodels.Order] {
	utils.LogAction(ctx, compOrdersStorage, "InsertOrder")

	if _, exists := s.orders[order.ID]; exists {
		return mo.Errf[dsmodels.Order]("order already exists")
	}
	s.orders[order.ID] = order
	return mo.Ok(order)
}
